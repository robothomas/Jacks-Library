package jackslib.util;

/**
 * <p>Borrowed from Tyler on FRC Team 3512
 * 
 * <p>{@link https://gist.github.com/calcmogul/302e73271f096d74bc46b3c50d2d9bc3#file-trapezoidalmotionprofile-java}
 * 
 * <p>A trapezoidal-shaped velocity profile.
 *
 ...
 *   new TrapezoidalMotionProfile.Constraints(kMaxV, kMaxA);
 * TrapezoidalMotionProfile.State previousProfiledReference = new TrapezoidalMotionProfile.State(initialReference, 0.0);
 * </code></pre>
 *
 * <p>Run on update:
 * <pre><code>
 * TrapezoidalMotionProfile profile =
 *   new TrapezoidalMotionProfile(constraints, unprofiledReference, previousProfiledReference);
...
public class TrapezoidalMotionProfile {
   ...
   */
  public TrapezoidalMotionProfile(Constraints constraints, State goal, State initial) {
    m_direction = shouldFlipAcceleration(initial, goal, constraints) ? -1 : 1;
   ...
   * @param goal        The desired state when the profile is complete.
   */
  public TrapezoidalMotionProfile(Constraints constraints, State goal) {
  ...
  public double timeLeftUntil(double target) {
    ...
    double decceleration = -m_constraints.maxAcceleration;
   ...
   *
   * <p>The profile is inverted if the velocity needs to decrease rather than
  ...
  private boolean shouldFlipAcceleration(State initial, State goal,
    ...
    double distanceChange = goal.position - initial.position;

    double t = Math.abs(velocityChange) / constraints.maxAcceleration;
    boolean isAccelerationFlipped = t * (velocityChange / 2 + initial.velocity) > distanceChange;
